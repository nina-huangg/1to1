...
const Calendar = ({ selectedDates }) => {
  ...
  const renderWeekDates = () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      dates.push(
        <div key={i} className="p-1 text-sm flex-grow text-center bg-gray-400">
  ...
  const generateTimeSlots = () => {
    ...
    for (let hour = 8; hour <= 17; hour++) {
      for (let min = 0; min < 60; min += 30) {
        ...
        timeSlots.push(
          <div key={`${time}`} className={`grid grid-cols-7 border p-1`}>
            {timeSlotCells}
          </div>
        );
      }
  ...
  const isSelectedTimeSlot = (time, day) => {
    return selectedTimeSlots.some(slot => slot.time === time && slot.day === day);
  };
